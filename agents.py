...
class CommuterAgent(mesa.Agent):
    ...
    def _pick_next_destination(self):
        ...
        if self.pos == self.home_node:
            # 80% go to work, 20% random errand weighted by population density
            self.destination = self.work_node if roll < 0.80 else int(np.random.choice(nodes, p=self.model.work_weights))
        elif self.pos == self.work_node:
            self.destination = self.home_node if roll < 0.80 else int(np.random.choice(nodes, p=self.model.home_weights))
        else:
    ...
    def step(self):
        ...
        if self.state == "evaluating":
            ...
            try:
                ...
                else: self.alight_node = self.destination
            except nx.NetworkXNoPath: self.alight_node = self.destination
            max_walk = 12
            prob_walk = 1.0 - (walk_distance / max_walk) if walk_distance < max_walk else 0.0
            if self.random.random() > prob_walk: self._take_auto() 
            else:
                self.walk_path = shortest_path[1:] 
                if not self.walk_path:
                    self.state = "waiting"; self.waiting_time = 0
                    self.model.stop_queues[self.pos].append(self)
                else: self.state = "walking_to_stop"
            return
        ...
        if self.state == "waiting":
            self.waiting_time += 1
            if self.waiting_time > self.patience: self._take_auto()

class BusAgent(mesa.Agent):
    def __init__(self, unique_id, model, route_nodes, color):
        super().__init__(unique_id, model)
        self.capacity = 40; self.passengers = []; self.route = route_nodes      
        self.current_route_index = 0; self.direction = 1; self.color = color 
    def step(self):
        passengers_to_drop = [p for p in self.passengers if p.alight_node == self.pos]
        for p in passengers_to_drop:
            p.state = "arrived"
            self.passengers.remove(p); self.model.grid.place_agent(p, self.pos)
        ...
        while len(current_queue) > 0 and len(self.passengers) < self.capacity:
            ...
            self.passengers.append(next_commuter); self.model.grid.remove_agent(next_commuter) 
        if len(self.route) > 1:
            next_index = self.current_route_index + self.direction
            if next_index >= len(self.route) or next_index < 0:
                self.direction *= -1; next_index = self.current_route_index + self.direction
            self.current_route_index = next_index
            self.model.grid.move_agent(self, self.route[self.current_route_index])